...
//I wanted to add clouds, but never got to it. Feel free to add them yourself
function Cloud() {
...
The usage of the format does not havr fo be in a javascript object (Ex: Avatar); it can be in a javascript class (Ex: Obstacle).
*/
const Background = {
  stack: [
    Sky,
...
const GameEngine = {
  ...
  startGame: function() {
    ...
    let j = 0;
    let m = 50
    //The animation of the game, which runs it at 50 frames per second
    var interval = setInterval(function() {
      ...
      if (j++ >=m) {
        j=0;
        m = rand(50, 80);
        Background.stack.push(new Obstacle(rand(10, 20), rand(20, 33)));
      }
  ...
  //readyGame is the function that places Avatar at the left end of the canvas, and moves it to the middle of the canvas/
  readyGame: function() {
